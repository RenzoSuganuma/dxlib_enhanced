...
void sceneTitle(float delta_time) {
	...
	SetFontSize(100);
	DrawFormatString(TITLE_LOGO_POS.x, TITLE_LOGO_POS.y, -1, TITLE_LOGO_STR.c_str());

	if (g_is_title_menu_on_draw) {
		SetFontSize(DEFAULT_FONT_SIZE);
		DrawFormatString(TITLE_MENU_POS.x, TITLE_MENU_POS.y, -1, TITLE_MENU_STR.c_str());
	}
...
void scenePlay(float delta_time) {
	...
	if (SEQ_SELECT_HAND == g_seq_scene_play) {
		...
		SetFontSize(DEFAULT_FONT_SIZE);
		DrawFormatString(PLAY_SEQ_SEL_HAND_STR_POS.x, PLAY_SEQ_SEL_HAND_STR_POS.y, -1, PLAY_SEQ_SEL_HAND_STR.c_str());
		...
		SetFontSize(30);
		DrawFormatString(PLAY_RESULT_CPU_HAND_POS.x, PLAY_RESULT_CPU_HAND_POS.y - 100, -1, "CPU");
		DrawFormatString(PLAY_RESULT_PLAYER_HAND_POS.x - 20, PLAY_RESULT_PLAYER_HAND_POS.y + 100, -1, "PLAYER");

		DrawFormatString(PLAY_RESULT_CPU_HAND_POS.x - 50, PLAY_RESULT_CPU_HAND_POS.y + 140, -1, RESULT_MESSAGE[g_reslut].c_str());
		...
		SetFontSize(DEFAULT_FONT_SIZE);
		DrawFormatString(PLAY_RESULT_PLAYER_HAND_POS.x - 130, PLAY_RESULT_PLAYER_HAND_POS.y + 220, -1, RETURN_TITLE_MASSAGE_STR.c_str());
	}
...
void gameStart() {
	...
	g_gpc_cursor_hdl = LoadGraph("resource/graphics/example/cursor.png");
	g_gpc_hand_hdls[HAND_TYPE_STONE] = LoadGraph("resource/graphics/example/stone.png");
	g_gpc_hand_hdls[HAND_TYPE_SCISSORS] = LoadGraph("resource/graphics/example/scissors.png");
	g_gpc_hand_hdls[HAND_TYPE_PAPER] = LoadGraph("resource/graphics/example/paper.png");
