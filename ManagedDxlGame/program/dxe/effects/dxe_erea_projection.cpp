...
namespace dxe {
        ...
        , const bool have_blur_tex)
    {
        ...
        vertex_shader_hdl_ = LoadVertexShader("resource/shader/dxe/erea_projection.vso");
        pixel_shader_hdl_ = LoadPixelShader("resource/shader/dxe/erea_projection.pso");
    ...
    void AreaProjectionTexture::refreshProcess() {
        ...
        camera_ = std::make_shared<dxe::Camera>((float)(size_.x * i_srv_dpi_divide), (float)(size_.y * i_srv_dpi_divide));
        camera_->setAngle(tnl::ToRadian(90));
    ...
    void AreaProjectionTexture::setCameraProjectionModePosition() {
        ...
        if (projection_mode_ == eProjectionMode::VOLUME_Y
            ...
            camera_->setPositionY(tnl::RightTriangleHeightByBase(tnl::ToRadian(45), float(size_.y * i_srv_dpi_divide / 2)));
            camera_->setTargetX(camera_->getPositionX());
            camera_->setTargetZ(camera_->getPositionZ());
            camera_->setUpper( { 0, 0, 1 } );
        }
    }
    ...
    void AreaProjectionTexture::update()
    {
        ...
        for (auto it : mesh_containers_) {
            ...
            tnl::Matrix proj = tnl::Matrix::OrthoLH(camera_->getScreenWidth(), camera_->getScreenHeight(), 1, 100000);

            AreaProjectionCbuffer* cbuff = (AreaProjectionCbuffer*)GetBufferShaderConstantBuffer(constant_buffer_hdl_);
            cbuff->view_ = XMLoadFloat4x4(&camera_->getViewMatrix());
